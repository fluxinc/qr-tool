...
function Get-DicomWorklist {
  ...
  try {
    # Log DICOM connection details for Worklist Query
    $modalityFilter = if ($global:WorklistModalityFilter) { "modality=$global:WorklistModalityFilter" } else { "" }
    $dateFilter = if ($global:WorklistScheduledDateFilter) { "date=$global:WorklistScheduledDateFilter" } else { "" }
    $queryParams = @($modalityFilter, $dateFilter) | Where-Object { $_ -ne "" }
    $queryString = if ($queryParams.Count -gt 0) { "?" + ($queryParams -join "&") } else { "" }
    Write-LogInfo "WORKLIST: dicom://$global:myAE:$global:WorklistEndpointAETitle@$global:WorklistEndpointHost:$global:WorklistEndpointPort/worklist$queryString"
    ...
    $worklistResponses = Invoke-WithRetry -ScriptBlock {
      ...
      Write-LogDebug "Attempting worklist connection to $global:WorklistEndpointHost:$global:WorklistEndpointPort (AE: $global:WorklistEndpointAETitle)"
      ...
      Write-LogDebug "Worklist query completed successfully. Received $($responses.Count) responses"
      return $responses
...
function Start-WorklistQueryProcess {
  ...
  # Always process existing items in the queues (regardless of new worklist items)
  # This ensures that items from previous failed Q/R operations get retried
  try {
    Write-LogInfo "Processing existing queued items"
    Do-Stage1  # Process incoming stored items
    Do-Stage2  # Query for studies and create move requests
    Do-Stage3  # Execute move requests
    Write-LogInfo "Completed processing existing queued items"
  }
  catch {
    Write-LogError "Error processing existing queued items: $_"
  }
}
